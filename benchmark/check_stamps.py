...
def parse_input(filename, remove_repeat=True):
        ...
        for line in fp:
            if line.startswith('#') or len(line.strip()) is 0:
                ...
                try:
                    stamp = float(s[0])
                    stamp2 = float(s[1])
                    stampa[seq].append(stamp)
                    stampb[seq].append(stamp2)
    ...
    for seq in stampa:
        print(seq)
        plt.subplot(1, len(stampa), seq)
        plt.plot(stampa[seq], stampb[seq], 'bo')
        plt.hold(True)
        plt.plot([min(stampa[seq]), max(stampa[seq])], [min(stampb[seq]), min(stampb[seq]) - min(stampa[seq]) + max(stampa[seq])], 'k')
    plt.show()
    exit(0)
