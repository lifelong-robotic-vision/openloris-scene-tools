...
class RGBDCreator():
    def __init__(self):
        ...
        if not os.path.isdir(save_folder + 'depth/'):
            os.mkdir(save_folder + 'depth/')
        if not os.path.isdir(save_folder + 'aligned_depth/'):
            os.mkdir(save_folder + 'aligned_depth/')
        timestr_1 = []
        timestr_2 = []
        timestr_3 = []
        with rosbag.Bag(args.inbag, 'r') as bag:
            ...
            for topic, msg, t in bag.read_messages():
                if topic == "/d400/color/image_raw":
                    ...
                    cv2.imwrite(save_folder+'color/'+image_name, cv_image)
                if topic == "/d400/depth/image_raw":
                    ...
                    image_name = timestr+ ".png"
                    cv2.imwrite(save_folder+'depth/'+image_name, cv_image)
                if topic == "/d400/aligned_depth_to_color/image_raw":
                    ...
                    image_name = timestr+ ".png"
                    cv2.imwrite(save_folder+'aligned_depth/'+image_name, cv_image)
        ...
        f3 = open(save_folder + fname + '.txt', 'wt')
        try:
        ...
        f4 = open(save_folder + fname + '.txt', 'wt')
        try:
...
class FisheyeCreator():
    def __init__(self):
        ...
        if not os.path.isdir(save_folder + 'fisheye2/'):
        ...
        with rosbag.Bag(args.inbag, 'r') as bag:
            ...
            for topic, msg, t in bag.read_messages():
                if topic == "/t265/fisheye1/image_raw":
                    ...
                    timestr_1.append(timestr)
                    image_name = timestr+ ".png"
                    cv2.imwrite(save_folder+'fisheye1/'+image_name, cv_image)
                if topic == "/t265/fisheye2/image_raw":
                    ...
                    timestr_2.append(timestr)
                    image_name = timestr+ ".png"
                    cv2.imwrite(save_folder+'fisheye2/'+image_name, cv_image)
        ...
        timestr_1.sort()
        timestr_2.sort()
        f1 = open(save_folder + "fisheye1.txt", "w")
        for timestr in timestr_1:
            f1.writelines(timestr + ' fisheye1/' + timestr + ".png\n")
        f1.close()
        f2 = open(save_folder + "fisheye2.txt", "w")
        for timestr in timestr_2:
...
class Odom_GT_Creator():
    def __init__(self):
        ...
        fname = 'odom'
        f1 = open(save_folder + fname + '.txt', 'wt')
        try:
        ...
        fname = 'groundtruth'
        f2 = open(save_folder + fname + '.txt', 'w')
...
if __name__ == '__main__':
    ...
    try:
        ...
        odom_gt_creator = Odom_GT_Creator()
